...
;
    ...
    Do(function(){
        ...
        $("#comments").delegate(".comment-report a", 'click', function (e) {
            ...
            var url = "https://www.douban.com/group/topic/232740917/?".concat(opt, '=', cid);
            var type = ['comment']
        ...
        $.post_withck(url, {},
            function(sjson){
                ...
                if (ret.result=="toomany"){
                    ...
                    $("#replysect").html('<br/><h2>你现在加入了这个小组，可以发表回应</h2><div class="txd comment-form"><form name="comment_form" method="post" action="add_comment"><textarea name="rv_comment" rows="8" cols="54"></textarea><br/><input type="hidden" name="start" value="200"/><span class="bn-flat-hot rr"><input type="submit" value="发送"/></span></form></div>');
                }
            });
        return false;
    });
